...
function SectionCard({ title, desc, children }: { title: string; desc: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl bg-[#0c0f1a] border border-white/8 p-6 space-y-5">
      <div>
  ...
  return (
    <div className="space-y-2">
      ...
      <input
        ...
        autoComplete="off"
        className="w-full px-4 py-3 rounded-xl bg-black/40 border border-white/10 text-white text-center tracking-[0.5em] text-base font-mono focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:border-amber-500/30 transition"
        placeholder="——————"
      />
    </div>
  );
...
export default function StaffSettingsPage() {
  ...
  async function setInitialPin() {
    ...
    setPinLoading(false);
    if (res.ok) {
      setPinMsg({ ok: true, text: "PIN set successfully. You can now block and unblock students." });
  ...
  async function changeExistingPin() {
    ...
    const verifyRes = await fetch("/api/staff/pin/verify", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="max-w-md space-y-6">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-0.5">Manage your staff PIN</p>
      </div>

      <SectionCard
        title="Set Staff PIN"
        desc="Required for blocking and unblocking students. Must be 6 digits."
      >
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-semibold text-sm transition active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {pinLoading ? (
              <>
      ...
      <div className="rounded-xl bg-amber-500/5 border border-amber-500/15 px-4 py-3">
        <p className="text-xs text-amber-400/80 leading-relaxed">
          The PIN is hashed with bcrypt and never stored in plain text. After 5 wrong attempts it locks — contact your administrator to reset. Never share your PIN.
        </p>
      </div>
    </div>
  );
