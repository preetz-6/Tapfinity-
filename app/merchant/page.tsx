  ...
  return (
    <div className={`rounded-2xl border bg-gradient-to-b from-white/[0.03] to-transparent p-5 flex items-start gap-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${accent ?? "border-white/[0.06] hover:border-white/[0.12] hover:shadow-black/20"}`}>
      ...
      <div className="min-w-0">
        <p className="text-xs text-gray-5100 font-semibold mb-1 tracking-wide">{label}</p>
...
export default function MerchantHome() {
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto space-y-8 py-2 px-6">
      ...
      {!loading && stats && (
        <>
          <div className="space-y-4">
            <p className="text-s text-gray-5000 uppercase tracking-widest font-bold">Revenue Overview</p>
      ...
      <div className="flex flex-col items-center gap-5 py-6 bg-gradient-to-b from-white/[0.01] to-white/[0.03] border border-white/[0.04] rounded-3xl p-8 shadow-xl shadow-black/20">
        <p className="text-m text-white-400 font-semibold uppercase tracking-wide"></p>
        <Link href="/merchant/receive" className="group">
          <div className="relative flex items-center justify-center w-48 h-48 cursor-pointer">
            {/* Pulsing rings */}
            <div className="absolute inset-0 rounded-full border border-violet-500/10 animate-ping opacity-60" style={{ animationDuration: "2s" }} />
            <div className="absolute inset-4 rounded-full border border-violet-500/15 animate-ping opacity-60" style={{ animationDuration: "2s", animationDelay: "0.5s" }} />
            <div className="absolute inset-8 rounded-full border border-violet-500/20 animate-ping opacity-60" style={{ animationDuration: "2s", animationDelay: "1s" }} />
